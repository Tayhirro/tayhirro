...
void EM_adc()
{
	  ...
	  uint16 i,j;
    int filter_temp;
	  //int Left[10],Left_Middle[10],Middle_L[10],Right[10],Right_Middle[10],Middle_R[10],Middle[10], Middle_H[10];
	  int Left_Middle[10],Right_Middle[10],Middle[10];
    ...
    {      
			  ...
			  Left_Middle[i]=adc_once(ADC_P01, ADC_12BIT);//采集10次求平均  分辨率12位
        //Middle_L[i]=adc_once(ADC_P01, ADC_12BIT);	//采集10次求平均  分辨率12位
			  //Middle_H[i]=adc_once(ADC_P00, ADC_12BIT);//采集10次求平均  分辨率12位
			  Middle[i]=adc_once(ADC_P05, ADC_12BIT);//采集10次求平均  分辨率12位
        //Middle_R[i]=adc_once(ADC_P14, ADC_12BIT);//采集10次求平均  分辨率12位
			  Right_Middle[i]=adc_once(ADC_P00, ADC_12BIT);//采集10次求平均  分辨率12位
 ...
 //   }
    ...
    for(j = 0; j < 10 - 1; j++)
    {
        for(i = 0; i < 10 - 1 - j; i++)
        {
            if(Left_Middle[i] > Left_Middle[i + 1])
            {
                filter_temp = Left_Middle[i];
                Left_Middle[i] = Left_Middle[i + 1];
                Left_Middle[i + 1] = filter_temp;
            }
        }
    }
    //左中
...
//    }
    ...
    for(j = 0; j < 10 - 1; j++)
    {
        for(i = 0; i < 10 - 1 - j; i++)
        {
            if(Right_Middle[i] > Right_Middle[i + 1])
            {
                filter_temp = Right_Middle[i];
                Right_Middle[i] = Right_Middle[i + 1];
                Right_Middle[i + 1] = filter_temp;
            }
        }
    }
    //右横
