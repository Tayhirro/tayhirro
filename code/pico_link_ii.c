...
#define UART_SEND_BYTES(data, length) uart_write_buffer((UART_3), (data), (length));
...
//----------------------发送缓冲区------------------------------
//默认是25000,如果要使用spi,把这个设置回去
#define BUFFER_SIZE (3000) // 发送缓冲区大小, 4字节倍数, 最大65536, 建议略大于图像字节数，太大会占用过多内存
