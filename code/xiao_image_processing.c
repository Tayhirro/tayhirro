        ...
        if (screen == IMAGE_IPS200) {
            ...
            if (Image_isUsefulData_Status) {
                if (showType == IMAGE_CLEAR_ORIGIN) {
                    ...
                    //清除右边线
                    for (int i = 0; i < Image_iptsRightNum_Bak; ++i) {
...
static void Image_FindLine_LeftHand_Adaptive_inv(uint8* image, uint8 block_size, int8 clip_value, uint8 x, uint8 y) {
    ...
    uint16 step = 0;
    uint8 dir = 0, turn = 0;
    while (step < Image_rptsLeftNum-1  && half < x && x < IMAGE_WIDTH - half - 1 && half < y && y < IMAGE_HEIGHT - half - 1 && turn < 4) {
        //使用自适应阈值法求解阈值
        //clip_value为经验常数,一般取2~5
        int16 localThres = 0;
        for (int16 dy = -half; dy <= half; ++dy) {
...
static void Image_FindLine_RightHand_Adaptive(uint8* image, int block_size, int clip_value, int x, int y) {
    ...
    uint16 step = 0;
    uint8 dir = 0, turn = 0;
    while (step < Image_iptsRightNum-1 && half < x && x < IMAGE_WIDTH - half - 1&& half < y && y < IMAGE_HEIGHT - half - 1 && turn < 4) {
        //使用自适应阈值法求解阈值
        //clip_value为经验常数,一般取2~5
        int16 localThres = 0;
        for (int16 dy = -half; dy <= half; ++dy) {
