...
#define IMAGE_LINE_MAX_NUM (130)         //图片边线像素点最大个数
//------------------------------------------------------------
//所有状态机
...
//1. 原图左右边线
//2. 边线左右数据
extern uint8   Image_iptsLeft[130][2];           //左边线坐标存储 - 0代表x坐标,1代表y坐标
extern uint8   Image_iptsRight[130][2];          //右边线坐标存储
extern uint8   Image_iptsLeftNum;               //左边线像素点个数
extern uint8   Image_iptsRightNum;              //右边线像素点个数


//逆透视变换曲线
extern uint8 Image_rptsLeft[130][2];             //左边线去畸变+逆透视变换坐标存储
extern uint8 Image_rptsRight[130][2];            //右边线去畸变+逆透视变换坐标存储
...
extern uint8 Image_rptsLeftb[130][2];            //三角滤波后的左边线坐标存储
extern uint8 Image_rptsRightb[130][2];           //三角滤波后的右边线坐标存储
extern uint8 Image_rptsLeftbNum;                //三角滤波后的左边线长度
extern uint8 Image_rptsRightbNum;               //三角滤波后的右边线长度
//点集三角滤波相关
extern uint8 Image_iptsLeftb[130][2];            //三角滤波后的左边线坐标存储
extern uint8 Image_iptsRightb[130][2];           //三角滤波后的右边线坐标存储
...
extern uint8 Image_rptsLefts[130][2];           //等距采样后的左边线坐标存储
extern uint8 Image_rptsRights[130][2];          //等距采样后的右边线坐标存储
extern uint8 Image_rptsLeftsNum;               //等距采样后的左边线长度
extern uint8 Image_rptsRightsNum;              //等距采样后的右边线长度
//等距采样相关
extern uint8 Image_iptsLefts[130][2];           //等距采样后的左边线坐标存储
extern uint8 Image_iptsRights[130][2];          //等距采样后的右边线坐标存储
...
extern float Image_rptsLefta[130];               //左边线对应点处的角度大小
extern float Image_rptsRighta[130];              //右边线对应点处的角度大小
extern uint8 Image_rptsLeftaNum;                //左边线点的个数
extern uint8 Image_rptsRightaNum;               //右边线点的个数
//边线局部角度变化率相关
extern float Image_iptsLefta[130];               //左边线对应点处的角度大小
extern float Image_iptsRighta[130];              //右边线对应点处的角度大小
extern uint8 Image_iptsLeftaNum;                //左边线点的个数
extern uint8 Image_iptsRightaNum;               //右边线点的个数

//------------------------------
//角度变化率非极大抑制相关
extern float Image_rptsLeftan[130];              //左边线区域最大角存储
extern float Image_rptsRightan[130];             //右边线区域最大角存储
extern uint8 Image_rptsLeftanNum;               //左边线点的个数
extern uint8 Image_rptsRightanNum;              //右边线点的个数
//------------------------------
extern float Image_iptsLeftan[130];              //左边线区域最大角存储
extern float Image_iptsRightan[130];             //右边线区域最大角存储
extern uint8 Image_iptsLeftanNum;               //左边线点的个数
extern uint8 Image_iptsRightanNum;              //右边线点的个数
//------------------------------
//左右变线跟踪相关
extern uint8 Image_rptsLeftc[130][2];            //左边线跟踪得到的中线数据
extern uint8 Image_rptsRightc[130][2];           //右边线跟踪得到的中线数据
